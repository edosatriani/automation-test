...
module.exports = {
    ...
    topMenu: {
        toggleSettings: async (page, repeat) => {
            if (!repeat){
                let elem = await page.findById(component.newMenu.menuTop.mainMenu.id);
                elem.click();
            }
    ...
    settings:{
        general: {
            ...
            admin: {
                ...
                saveSettings : async (page) => {
                    el = await page.findById(component.newMenu.leftMenu.general.admin.saveSettings.id);
                    await el.click();
                    el = await page.findById(component.newMenu.leftMenu.general.admin.saveConfirmButton.id);
                    let attr = await el.getAttribute('id')
                    await el.click();
                    return attr;
                }
            }
